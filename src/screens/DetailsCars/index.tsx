...
export const DetailsCars = () => {
  ...
  const [carActive, setCarActive] = useState<number>(carSelected?.photos.length === 1 ? 0 : 1);
  ...
  const changeCarActiveColor = useCallback(
    (index: number) => {
      setCarActive(index);
    },
    []
      ...
      <s.Content>
        <s.Container_Type>
          ...
          <s.Container_Details>
            ...
            <s.Price>${carSelected?.price}/{carSelected?.period}</s.Price>
          </s.Container_Details>
        </s.Container_Type>
        <s.Container_Body>
